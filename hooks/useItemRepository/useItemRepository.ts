...
/**
 * useItemRepository - Hook to manage item repository operations
 * @param props - IItemRepositoryProps
 * @returns Object containing item repository methods
 */
export function useItemRepository(props: IItemRepositoryProps) {
  ...
  /**
   * getAllItems - Fetch all items with optional filters
   * @param options - Filter options
   * @returns Promise resolving to an array of Item instances
   */
  const getAllItems = React.useCallback(async (options: {
    withArchived?: boolean,
    withDocuments?: boolean,
    withNotification?: boolean,
    byArchived?: boolean,
    byCategoryIds?: string[],
    byLabelOrBrand?: string;
  } = {}): Promise<Item[]> => {
    const dynamicProps = [];
    let query = `SELECT * FROM items WHERE owner_id = ? `;
    dynamicProps.push(props.ownerId);
    if (!options.withArchived && options.byArchived === undefined) {
      query += 'AND is_archived = 0 ';
    }
    if (options.byArchived !== undefined) {
      query += 'AND is_archived = ? ';
      dynamicProps.push(options.byArchived ? 1 : 0);
    }
  ...
  /**
   * getItemsDocuments - Fetch documents attached to given item IDs
  ...
  /**
   * isDocumentAttachedToOtherItem - Check if a document is attached to other items
   * @param documentId - Document ID to check
   * @param currentItemId - Current item ID to exclude from the check
   * @returns Promise resolving to a boolean indicating if the document is attached to other items
   */
   ...
   * @param id - Item ID
   * @returns Promise resolving to an Item instance or null if not found
   */
  ...
  /**
   * saveItem - Save or update an item in the database
   * @param item - Item instance to save
   * @returns Promise resolving to the saved Item instance
   */
  ...
  /**
   * deleteItem - Delete an item and its associated data from the database
  ...
  /**
   * archiveItem - Archive an item by setting its is_archived flag to true
   * @param item - Item instance to archive
   * @returns Promise resolving when the item is archived
   */
  const archiveItem = React.useCallback(async (item: Item) => {
    return await db.withTransactionAsync(async () => {
      const statement = await db.prepareAsync(`UPDATE items SET is_archived = 1 WHERE id = $itemId AND owner_id = $ownerId`);
      try {
        statement.executeAsync({
          $itemId: item.getId(),
          $ownerId: props.ownerId
        ...
        throw new DatabaseSaveException(`Failed to archive item with id ${item.getId()} for user ${props.ownerId}`);
      }
    });
  }, [db, props.ownerId]);
  ...
  return React.useMemo(() => ({
    ...
    deleteItem,
    archiveItem
  }), [getAllItems, getItemById, saveItem, deleteItem, archiveItem]);
}
